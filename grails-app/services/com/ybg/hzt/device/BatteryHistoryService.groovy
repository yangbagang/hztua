...
class BatteryHistoryService {
    ...
    def calculateBySecond(Long batteryId, String key) {
        ...
        def query = "select create_second as xValue, round(avg(${key}),3) as yValue from battery_history where " +
    ...
    def calculateByMinute(Long batteryId, String key) {
        ...
        def startStr = sdf.format(startTime)
        def endStr = sdf.format(endTime)
        def query = "select create_minute as xValue, round(avg(${key}),3) as yValue from battery_history where " +
    ...
    def calculateByHour(Long batteryId, String key) {
        ...
        def startStr = sdf.format(startTime)
        def endStr = sdf.format(endTime)
        def query = "select create_hour as xValue, round(avg(${key}),3) as yValue from battery_history where " +
    ...
    def calculateByDay(Long batteryId, String key) {
        ...
        def startStr = sdf.format(startTime)
        def endStr = sdf.format(endTime)
        def query = "select create_day as xValue, round(avg(${key}),3) as yValue from battery_history where " +
    ...
    def calculateByMonth(Long batteryId, String key) {
        ...
        def startStr = sdf.format(startTime)
        def endStr = sdf.format(endTime)
        def query = "select create_month as xValue, round(avg(${key}),3) as yValue from battery_history where " +
    ...
    def calculateByYear(Long batteryId, String key) {
        ...
        def startStr = sdf.format(startTime)
        def endStr = sdf.format(endTime)
        def query = "select create_year as xValue, round(avg(${key}),3) as yValue from battery_history where " +
